...
const applyTuning = async () => {
  ...
  isSaved.value = true;
  statusMsg.value = `Applied & Saved to DB: Kp=${kp.value}, Ki=${ki.value}, Kd=${kd.value}, Speed=${forwardSpeed.value}m/s`;
...
<template>
  <div class="glass-card p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
       ...
       <div class="space-y-2 bg-(--bg-secondary) p-4 rounded-xl border border-(--border-primary)">
          <div class="flex justify-between items-center text-[10px] font-black uppercase">
             ...
             <span class="font-mono text-sm text-(--text-primary)">{{ kp }}</span>
          </div>
          <input type="range" min="0" max="50" step="0.001" v-model.number="kp" class="w-full accent-primary cursor-pointer" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
       ...
       <div class="space-y-2 bg-(--bg-secondary) p-4 rounded-xl border border-(--border-primary)">
          <div class="flex justify-between items-center text-[10px] font-black uppercase">
             <span class="text-(--text-secondary)">Cruise Forward Speed (m/s)</span>
             <span class="font-mono text-sm text-primary">{{ forwardSpeed.toFixed(1) }} m/s</span>
          </div>
          <input type="range" min="0.2" max="3.0" step="0.1" v-model.number="forwardSpeed" class="w-full accent-primary cursor-pointer" />
       </div>
       ...
       <div class="space-y-2 bg-(--bg-secondary) p-4 rounded-xl border border-(--border-primary)">
          ...
          <input type="range" min="1" max="100" step="1" v-model.number="alignThresholdPx" class="w-full accent-primary cursor-pointer" />
       </div>
    </div>
