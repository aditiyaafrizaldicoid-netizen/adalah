...
const vectoredCmd = computed(() => {
  ...
  return {
    ...
    servo_left: mapPwm(sl),
    servo_right: mapPwm(sr)
  };
});
  ...
  <div class="h-full flex flex-col p-6 space-y-6 overflow-y-auto">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-(--text-secondary) mt-1">Kendalikan pergerakan ASV menggunakan Gamepad atau tombol keyboard <b>WASD</b>.</p>
      </div>
    </div>
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-(--bg-card) border border-(--border-primary) rounded-xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div class="grid grid-cols-2 gap-4 relative z-10">
            <div class="bg-(--bg-secondary) rounded-lg p-4 border border-(--border-subtle)">
              <div class="text-xs text-(--text-secondary) mb-1 font-semibold uppercase tracking-wider">Throttle (Maju/Mundur)</div>
            ...
            <div class="bg-(--bg-secondary) rounded-lg p-4 border border-(--border-subtle)">
              <div class="text-xs text-(--text-secondary) mb-1 font-semibold uppercase tracking-wider">Steering (Belok)</div>
              <div class="flex items-end gap-2">
        ...
        <div class="bg-(--bg-card) border border-(--border-primary) rounded-xl p-6 shadow-xl">
          ...
          <div class="mt-4 text-xs text-warning p-3 rounded-lg border border-warning/20">
            <strong>Catatan:</strong> Output di atas akan dikirimkan sebanyak <b>{{ updateRateHz }} kali per detik</b> ke Pixhawk hanya jika ASV dalam keadaan <strong>ARMED</strong>.
          </div>
        </div>
      </div>
